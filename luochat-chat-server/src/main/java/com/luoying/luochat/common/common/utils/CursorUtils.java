...
public class CursorUtils {
    ...
    /**
     *
     * @param mapper mybatisplus的数据库操作类，让工具类能调用一些基础的查询方法。
     * @param request 游标翻页请求对象（前端传过来的游标分页请求对象，里面的字段是cursor和size）
     * @param initWrapper 自定义查询条件扩展点（每张表的额外查询条件不一样，需要调用者自己构造）
     * @param cursorColumn 游标字段（每张表的游标字段不一样，需要调用者传）
     * @return
     * @param <T>
     */
